...
\begin{itemize}
    ...
    % Parámetros de almacenamiento (batería del agente estratégico $i$)
    \item $\rho$: estado inicial de carga de la batería.
    \item $\eta^{ch}$: eficiencia de carga de la batería.
    \item $\eta^{dis}$: eficiencia de descarga de la batería.
    \item $E^{\max}$: capacidad máxima de almacenamiento de la batería.
    \item $P^{ch,\max}$: potencia máxima de carga.
    \item $P^{dis,\max}$: potencia máxima de descarga.
\end{itemize}

Variables:
\begin{itemize}
    \item $q_{j,t}$: energía ofertada por el generador $j$ en hora $t$.
    \item $d_{l,t}$: demanda atendida del consumidor $l$ en hora $t$.
    \item $SOC_t$: estado de carga de la batería del agente estratégico $i$ en hora $t$.
    \item $ch_t$: energía cargada en la batería en hora $t$.
    \item $dis_t$: energía descargada de la batería en hora $t$.
\end{itemize}

\textit{La batería pertenece al generador estratégico $i$ y puede almacenar energía que no venda en el mercado spot.}
...
\label{LF_obj}\\[2mm]
\text{s.a.}\;\;
& \sum_{j} q_{j,t} + dis_t \;-\; \sum_{l} d_{l,t}  \;=\; 0 
...
\label{LF_cap_others}\\
& 0 \le q_{i,t}
\quad (\perp\; \omega^{q,\min}_{i,t})
\quad \forall t
\label{LF_cap_i_min}\\
& q_{i,t} + ch_t \le (1-x_{i,y(t)})\,Q_{i,t}
\quad (\perp\; \omega^{q,\max}_{i,t})
\quad \forall t
\label{LF_cap_i_max}\\
...
\label{LF_dem}\\
% Restricciones de almacenamiento (batería del agente estratégico i)
& SOC_1 = \rho + \eta^{ch} ch_1 - \frac{1}{\eta^{dis}} dis_1 \quad t=1 \\
& SOC_t = SOC_{t-1} + \eta^{ch} ch_t - \frac{1}{\eta^{dis}} dis_t \quad \forall t=2,\dots,T \\
& 0 \leq SOC_t \leq E^{\max} \quad \forall t \\
& 0 \leq ch_t \leq P^{ch,\max} \quad \forall t \\
& 0 \leq dis_t \leq P^{dis,\max} \quad \forall t \\
& SOC_T = \rho \\
\end{align}
...
- \alpha_{i,t}\, dis_t
- \sum_{j,t} \alpha_{j,t}\, q_{j,t} \\
& + \sum_{t} \lambda_t \left( dis_t + \sum_{j} q_{j,t} - \sum_{l} d_{l,t} \right) \\
& + \sum_{j \neq i,\, t} \omega^{q,\min}_{j,t}\, q_{j,t}
+ \sum_{j \neq i,\, t} \omega^{q,\max}_{j,t}\, (Q_{j,t} - q_{j,t}) \\
& + \sum_{t} \omega^{q,\min}_{i,t}\, q_{i,t}
+ \sum_{t} \omega^{q,\max}_{i,t}\, \left( (1-x_{i,y(t)}) Q_{i,t} - q_{i,t} - ch_t \right) \\
& + \sum_{l,t} \omega^{d,\min}_{l,t}\, d_{l,t}
+ \sum_{l,t} \omega^{d,\max}_{l,t}\, (D_{l,t} - d_{l,t}) \\
& + \mu_1 (SOC_1 - \rho - \eta^{ch} ch_1 + \frac{1}{\eta^{dis}} dis_1) \\
& + \sum_{t=2}^T \mu_t (SOC_t - SOC_{t-1} - \eta^{ch} ch_t + \frac{1}{\eta^{dis}} dis_t) \\
& + \sum_{t} \phi_t^{SOC,\min} SOC_t + \sum_{t} \phi_t^{SOC,\max} (E^{\max} - SOC_t) \\
& + \sum_{t} \phi_t^{ch,\min} ch_t + \sum_{t} \phi_t^{ch,\max} (P^{ch,\max} - ch_t) \\
& + \sum_{t} \phi_t^{dis,\min} dis_t + \sum_{t} \phi_t^{dis,\max} (P^{dis,\max} - dis_t) \\
& + \psi (SOC_T - \rho)
\end{align}

Donde $\mu_t$, $\phi_t^{SOC,\min}$, $\phi_t^{SOC,\max}$, $\phi_t^{ch,\min}$, $\phi_t^{ch,\max}$, $\phi_t^{dis,\min}$, $\phi_t^{dis,\max}$ y $\psi$ son los multiplicadores de Lagrange asociados a las restricciones de almacenamiento.
...
\begin{itemize}
    ...
    \begin{align}
        &\forall t: && dis_t + \sum_{j} q_{j,t} - \sum_{l} d_{l,t} = 0 \\
        &\forall j \neq i,\, \forall t: && 0 \leq q_{j,t} \leq Q_{j,t} \\
        &\forall t: && 0 \leq q_{i,t} + ch_t \leq (1-x_{i,y(t)}) Q_{i,t} \\
        &\forall l,\, \forall t: && 0 \leq d_{l,t} \leq D_{l,t} \\
        & t=1: && SOC_1 = \rho + \eta^{ch} ch_1 - \frac{1}{\eta^{dis}} dis_1 \\
        &\forall t=2,\dots,T: && SOC_t = SOC_{t-1} + \eta^{ch} ch_t - \frac{1}{\eta^{dis}} dis_t \\
        &\forall t: && 0 \leq SOC_t \leq E^{\max} \\
        &\forall t: && 0 \leq ch_t \leq P^{ch,\max} \\
        &\forall t: && 0 \leq dis_t \leq P^{dis,\max} \\
        & && SOC_T = \rho
    \end{align}
    ...
    \begin{align}
        ...
        &\forall t: && \omega^{q,\max}_{i,t} \cdot (q_{i,t} + ch_t - (1-x_{i,y(t)}) Q_{i,t}) = 0 \\
        &\forall l,\, t: && \omega^{d,\min}_{l,t} \cdot d_{l,t} = 0 \\
        &\forall l,\, t: && \omega^{d,\max}_{l,t} \cdot (d_{l,t} - D_{l,t}) = 0 \\
        &\forall t: && \phi_t^{SOC,\min} \cdot SOC_t = 0 \\
        &\forall t: && \phi_t^{SOC,\max} \cdot (E^{\max} - SOC_t) = 0 \\
        &\forall t: && \phi_t^{ch,\min} \cdot ch_t = 0 \\
        &\forall t: && \phi_t^{ch,\max} \cdot (P^{ch,\max} - ch_t) = 0 \\
        &\forall t: && \phi_t^{dis,\min} \cdot dis_t = 0 \\
        &\forall t: && \phi_t^{dis,\max} \cdot (P^{dis,\max} - dis_t) = 0
    \end{align}
    ...
    \begin{align}
        ...
        &\forall t: && \phi_t^{SOC,\min} \geq 0,\quad \phi_t^{SOC,\max} \geq 0 \\
        &\forall t: && \phi_t^{ch,\min} \geq 0,\quad \phi_t^{ch,\max} \geq 0 \\
        &\forall t: && \phi_t^{dis,\min} \geq 0,\quad \phi_t^{dis,\max} \geq 0
    \end{align}

    \item \textbf{Condiciones de estacionalidad:}
    Por último, para encontrarnos en el óptimo las derivadas parciales del Lagrangiano respecto a las variables del problema inferior deben ser 0. Estas variables son: $q_{i,t}, q_{j,t}, d_{l,t}, SOC_t, ch_t, dis_t$.
    \begin{align}
        ...
        &\forall t: && \begin{cases}
            \mu_1 + \phi_1^{SOC,\min} - \phi_1^{SOC,\max} = 0 & \text{si } t=1 \\
            \mu_t - \mu_{t+1} + \phi_t^{SOC,\min} - \phi_t^{SOC,\max} = 0 & \text{si } 1 < t < T \\
            \mu_T + \phi_T^{SOC,\min} - \phi_T^{SOC,\max} + \psi = 0 & \text{si } t=T
        \end{cases} \\
        &\forall t: && - \mu_t \eta^{ch} + \omega^{q,\max}_{i,t} + \phi_t^{ch,\min} - \phi_t^{ch,\max} = 0 \\
        &\forall t: && - \alpha_{i,t} + \lambda_t + \mu_t \frac{1}{\eta^{dis}} + \phi_t^{dis,\min} - \phi_t^{dis,\max} = 0
    \end{align}

\end{itemize}

\subsection{Linealización de las condiciones de complementariedad: big-M}

\subsubsection{Linealización de las condiciones de complementariedad en variables $\omega$}
...
\begin{itemize}
    ...
    \begin{align}
        ...
        &q_{i,t} + ch_t \leq M \cdot (1 - z^{q,\min}_{i,t}) \\
...
\subsubsection{Linealización de las condiciones de complementariedad en variables $\phi$}

Para las condiciones de complementariedad asociadas a las variables de almacenamiento ($\phi$), se utiliza el mismo esquema big-M que para las $\omega$. Para cada restricción de almacenamiento:

\begin{itemize}
    \item \textbf{Estado de carga $SOC_t$:}
    \begin{align}
        &\phi_t^{SOC,\min} \leq M^{SOC,\min} \cdot z_t^{SOC,\min} \\
        &SOC_t \leq M^{SOC,\min} \cdot (1 - z_t^{SOC,\min}) \\
        &\phi_t^{SOC,\max} \leq M^{SOC,\max} \cdot z_t^{SOC,\max} \\
        &E^{\max} - SOC_t \leq M^{SOC,\max} \cdot (1 - z_t^{SOC,\max}) \\
        &z_t^{SOC,\min},\; z_t^{SOC,\max} \in \{0,1\}
    \end{align}
    \item \textbf{Carga $ch_t$:}
    \begin{align}
        &\phi_t^{ch,\min} \leq M^{ch,\min} \cdot z_t^{ch,\min} \\
        &ch_t \leq M^{ch,\min} \cdot (1 - z_t^{ch,\min}) \\
        &\phi_t^{ch,\max} \leq M^{ch,\max} \cdot z_t^{ch,\max} \\
        &P^{ch,\max} - ch_t \leq M^{ch,\max} \cdot (1 - z_t^{ch,\max}) \\
        &z_t^{ch,\min},\; z_t^{ch,\max} \in \{0,1\}
    \end{align}
    \item \textbf{Descarga $dis_t$:}
    \begin{align}
        &\phi_t^{dis,\min} \leq M^{dis,\min} \cdot z_t^{dis,\min} \\
        &dis_t \leq M^{dis,\min} \cdot (1 - z_t^{dis,\min}) \\
        &\phi_t^{dis,\max} \leq M^{dis,\max} \cdot z_t^{dis,\max} \\
        &P^{dis,\max} - dis_t \leq M^{dis,\max} \cdot (1 - z_t^{dis,\max}) \\
        &z_t^{dis,\min},\; z_t^{dis,\max} \in \{0,1\}
    \end{align}
\end{itemize}

\paragraph{}
\textbf{Elección del parámetro $M$ para almacenamiento}

Al igual que en el caso de las variables $\omega$, el valor de $M$ debe ser suficientemente grande para no restringir el espacio factible, pero no excesivamente grande para evitar problemas numéricos. Se recomienda:
\begin{itemize}
    \item Para las variables primales: usar las cotas naturales de cada variable ($E^{\max}$ para $SOC_t$, $P^{ch,\max}$ para $ch_t$, $P^{dis,\max}$ para $dis_t$).
    \item Para los multiplicadores duales ($\phi$): un valor razonable es el máximo impacto que puede tener la restricción en el objetivo, por ejemplo, el precio máximo de la energía en el mercado o el máximo coste marginal de generación.
\end{itemize}
